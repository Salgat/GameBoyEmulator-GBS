...
sf::Image GameBoy::RenderFrame() {
	...
	do {
		...
		if (mmu.interrupt_enable and cpu.interrupt_master_enable and if_memory_value) {
			...
			if (interrupt_fired & 1) {if_memory_value &= 0XFD; cpu.RST48();}
			else if (interrupt_fired & 2) {if_memory_value &= 0XFB; cpu.RST50();}
			else if (interrupt_fired & 4) {if_memory_value &= 0XF7; cpu.RST58();}
			else if (interrupt_fired & 8) {if_memory_value &= 0XEF; cpu.RST60();}
		...
		// Todo: Timer management (such as increment)
