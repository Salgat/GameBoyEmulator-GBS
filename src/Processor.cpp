...
void Processor::ExecuteNextInstruction() {
	...
	//std::cout << "Executing opcode at program counter: " << static_cast<unsigned int>(memory_value) << ", " << program_counter.word-1 << std::endl;
...
void Processor::LDrr_ld() {HL.lower  = DE.lower;  m_clock = 1;}
void Processor::LDrr_le() {HL.lower  = DE.higher; m_clock = 1;}
...
void Processor::ADCr_b() {uint16_t copy = AF.higher; AF.higher += BC.higher; AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+BC.higher>0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + BC.higher & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_c() {uint16_t copy = AF.higher; AF.higher += BC.lower;  AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+BC.lower >0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + BC.lower  & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_d() {uint16_t copy = AF.higher; AF.higher += DE.higher; AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+DE.higher>0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + DE.higher & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_e() {uint16_t copy = AF.higher; AF.higher += DE.lower;  AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+DE.lower >0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + DE.lower  & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_h() {uint16_t copy = AF.higher; AF.higher += HL.higher; AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+HL.higher>0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + HL.higher & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_l() {uint16_t copy = AF.higher; AF.higher += HL.lower;  AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+HL.lower >0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + HL.lower  & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
void Processor::ADCr_a() {uint16_t copy = AF.higher; AF.higher += AF.higher; AF.higher += (AF.lower&0x10)?1:0; AF.lower = (copy+AF.higher>0xFF)?0x10:0; if(!AF.higher) AF.lower |= 0x80; if ((copy & 0xF + AF.higher & 0xF) & 0x10) AF.lower |= 0x20; m_clock = 1;}
...
void Processor::SUBr_b() {int result = static_cast<int>(AF.higher) - static_cast<int>(BC.higher); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + BC.higher & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= BC.higher; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_c() {int result = static_cast<int>(AF.higher) - static_cast<int>(BC.lower);  AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + BC.lower  & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= BC.lower;  if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_d() {int result = static_cast<int>(AF.higher) - static_cast<int>(DE.higher); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + DE.higher & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= DE.higher; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_e() {int result = static_cast<int>(AF.higher) - static_cast<int>(DE.lower);  AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + DE.lower  & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= DE.lower;  if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_h() {int result = static_cast<int>(AF.higher) - static_cast<int>(HL.higher); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + HL.higher & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= HL.higher; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_l() {int result = static_cast<int>(AF.higher) - static_cast<int>(HL.lower);  AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + HL.lower  & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= HL.lower;  if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBr_a() {int result = static_cast<int>(AF.higher) - static_cast<int>(AF.higher); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + AF.higher & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= AF.higher; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBHL() {uint8_t memory_value = mmu->ReadByte(HL.word); int result = static_cast<int>(AF.higher) - static_cast<int>(memory_value); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + memory_value & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= memory_value; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
void Processor::SUBn() {uint8_t memory_value = mmu->ReadByte(program_counter.word++); int result = static_cast<int>(AF.higher) - static_cast<int>(memory_value); AF.lower = (result<0)?0x50:0x40; if((AF.higher & 0xF + memory_value & 0xF) & 0x10) AF.lower |= 0x20; AF.higher -= memory_value; if (!AF.higher) AF.lower |= 0x80; m_clock = 1;}
...
void Processor::SBCr_c() {uint16_t copy = AF.higher; AF.higher -= BC.lower;  AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-BC.lower <0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < BC.lower  & 0xF) AF.lower |= 0x20; m_clock = 1;}
void Processor::SBCr_d() {uint16_t copy = AF.higher; AF.higher -= DE.higher; AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-DE.higher<0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < DE.higher & 0xF) AF.lower |= 0x20; m_clock = 1;}
void Processor::SBCr_e() {uint16_t copy = AF.higher; AF.higher -= DE.lower;  AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-DE.lower <0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < DE.lower  & 0xF) AF.lower |= 0x20; m_clock = 1;}
void Processor::SBCr_h() {uint16_t copy = AF.higher; AF.higher -= HL.higher; AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-HL.higher<0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < HL.higher & 0xF) AF.lower |= 0x20; m_clock = 1;}
void Processor::SBCr_l() {uint16_t copy = AF.higher; AF.higher -= HL.lower;  AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-HL.lower <0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < HL.lower  & 0xF) AF.lower |= 0x20; m_clock = 1;}
void Processor::SBCr_a() {uint16_t copy = AF.higher; AF.higher -= AF.higher; AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-AF.higher<0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < AF.higher & 0xF) AF.lower |= 0x20; m_clock = 1;}
...
void Processor::SBCn() {uint16_t copy = AF.higher; uint8_t memory_value = mmu->ReadByte(program_counter.word++); AF.higher -= memory_value; AF.higher -= (AF.lower&0x10)?1:0; AF.lower = (copy-memory_value<0)?0x50:0x40; if(!AF.higher) AF.lower |= 0x80; if (copy & 0xF < memory_value & 0xF) AF.lower |= 0x20; m_clock = 2;}
...
void Processor::ANDr_b() {AF.higher &= BC.higher; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_c() {AF.higher &= BC.lower ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_d() {AF.higher &= DE.higher; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_e() {AF.higher &= DE.lower;  AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_h() {AF.higher &= HL.higher; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_l() {AF.higher &= HL.lower;  AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDr_a() {AF.higher &= AF.higher; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDHL() {AF.higher &= mmu->ReadByte(HL.word); AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::ANDn() {AF.higher &= mmu->ReadByte(program_counter.word++); AF.lower = AF.higher?0:0x80; m_clock = 1;}
...
void Processor::INCr_b() {BC.higher++; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_c() {BC.lower++ ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_d() {DE.higher++; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_e() {DE.lower++ ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_h() {HL.higher++; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_l() {HL.lower++ ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCr_a() {AF.higher++; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::INCHLm() {uint8_t memory_value = mmu->ReadByte(HL.word); mmu->WriteByte(HL.word, ++memory_value); AF.lower = memory_value?0:0x80; m_clock = 3;}

void Processor::DECr_b() {BC.higher--; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_c() {BC.lower-- ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_d() {DE.higher--; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_e() {DE.lower-- ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_h() {HL.higher--; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_l() {HL.lower-- ; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECr_a() {AF.higher--; AF.lower = AF.higher?0:0x80; m_clock = 1;}
void Processor::DECHLm() {uint8_t memory_value = mmu->ReadByte(HL.word); mmu->WriteByte(HL.word, --memory_value); AF.lower = memory_value?0:0x80; m_clock = 3;}
...
void Processor::CPL() {AF.higher ^= AF.higher; AF.lower |= 0x60; m_clock = 1;}
//void Processor::NEG() {}

// Complement carry flag and Set carry flag
void Processor::CCF() {uint8_t complemented_carry = (AF.lower&0x10)?0:0x10; AF.lower = AF.lower&0x80 + complemented_carry; m_clock = 1;}
void Processor::SCF() {AF.lower |= 0x10; m_clock = 1;}
...
void Processor::POPAF() {AF.lower = mmu->ReadByte(stack_pointer.word++); AF.higher = mmu->ReadByte(stack_pointer.word++); m_clock = 3;}
...
void Processor::JRNZn() {int16_t memory_value = mmu->ReadByte(program_counter.word); if (memory_value > 127) memory_value-=(~memory_value+1); ++program_counter.word; m_clock = 2; if(!AF.lower&0x80) {program_counter.word += memory_value; ++m_clock;}}
...
void Processor::DI() {interrupt_master_enable = 0; m_clock = 1;}
void Processor::EI() {interrupt_master_enable = 1; m_clock = 1;}
